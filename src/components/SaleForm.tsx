...
import QrScanner from "react-qr-scanner";           // ← این را اضافه کن
import { Sale, useSaleStore } from "../stores/saleStore";

type FS = Omit<Sale, "id" | "createdAt">;
interface Props { initial?: Sale | null; onClose: () => void; }
...
export default function SaleForm({ initial, onClose }: Props) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (!form.invoiceNumber.trim())
      return setErr({ invoiceNumber: t("required") });
    if (!form.customerName.trim())
      return setErr({ customerName: t("required") });
            ...
            <>
              <QrScanner
                ...
                onScan={data => {
                  if (data) {
                    try {
                      const obj = JSON.parse((data as any).text);
